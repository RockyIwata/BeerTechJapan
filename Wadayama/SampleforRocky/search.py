...
from db_for_MVP import db_session, Store, BrandLogo
import webbrowser
import os
...
if not stores_info:
    ...
    current_lat, current_lng = 35.69261665654802, 139.76240992772065  # 仮のデフォルト現在地
...
if stores_info:
    ...
    # foliumマップの作成
    map = folium.Map(location=[stores_info[0][0], stores_info[0][1]], zoom_start=16)  # 店舗情報の最初の店舗の位置を中心とする
    
    # 同じbrand毎に異なる色を割り当てるための色のマッピング
    color_map = {'ヱビス樽生': 'blue', 'エーデル樽生': 'green', 'ソラチ樽生': 'yellow', '黒ラベル樽生': 'black', 'プレミアムモルツ': 'orange', '琥珀エビス': 'red', '樽生ブラック': 'white'}
    ...
    # 地図を一時ファイルとして保存し、ブラウザで開く
    temp_map_path = "temp_map.html"
    map.save(temp_map_path)
    webbrowser.open("file://" + os.path.realpath(temp_map_path))
    
    print("マップがブラウザで表示されました。")
else:
